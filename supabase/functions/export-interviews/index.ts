...
Deno.serve(async (req: Request) => {
    ...
    const profiles = new Map(
        (profilesRes.data ?? []).map((p: { id: string; full_name: string; email: string; phone: string }) => [p.id, p])
    );
    ...
    const sorted = [...rows.values()].sort(
        (a, b) => a.domain.localeCompare(b.domain) || a.fullName.localeCompare(b.fullName)
    );
