...
export function AdminInterviews() {
  ...
  const rows = React.useMemo<InterviewRow[]>(() => {
    ...
    return [...map.values()].sort((a, b) => a.domain_name.localeCompare(b.domain_name) || a.full_name.localeCompare(b.full_name));
    ...
    <div className="page py-8">
      ...
      {!loading && !error && data && (
        <section className="panel panel-ticks relative">
          <div className="flex flex-col gap-4 border-b border-border p-5 sm:flex-row sm:items-end sm:p-6">
            ...
            <div className="w-full sm:max-w-xs">
              ...
              <div className="relative mt-1.5">
                <Search className="pointer-events-none absolute left-2.5 top-1/2 size-4 -translate-y-1/2 text-muted-foreground/60" aria-hidden="true" />
                <Input
                  id="iv-search"
                  value={search}
            ...
            <p className="font-mono text-[0.625rem] uppercase tracking-[0.05em] text-muted-foreground sm:ml-auto">
            ...
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead className="bg-secondary">
                  <tr>
                    {["Student", "Email", "Phone", "Domain", "Interview", "Selection", ""].map((h, i) => (
                      <th
                        key={h}
                        scope="col"
                        className={cn(
                          "px-5 py-3 text-left font-mono text-[0.6875rem] font-medium uppercase tracking-[0.05em] text-muted-foreground",
                          i === 0 && "pl-6",
                        )}
                      >
                        {h}
                ...
                <tbody className="divide-y divide-border">
                  {filtered.map((row) => (
                    <tr key={`${row.student_id}-${row.domain_id}`} className="transition-colors duration-150 hover:bg-secondary/60">
                      <td className="whitespace-nowrap px-5 py-4 pl-6">
                        <div className="flex items-center gap-3">
                          <div>
                            <p className="text-sm font-medium text-foreground">{row.full_name || "—"}</p>
                            <p className="font-mono text-[0.625rem] uppercase tracking-[0.05em] text-muted-foreground">{row.course || "—"}</p>
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="whitespace-nowrap px-5 py-4">
                        <label className="flex cursor-pointer items-center gap-2">
                          ...
                          />
                          <span className={cn("text-sm", row.interview_done ? "text-success" : "text-muted-foreground")}>
                      ...
                      <td className="whitespace-nowrap px-5 py-4">
                        <label className="flex cursor-pointer items-center gap-2">
                          ...
                          />
                          <span className={cn("text-sm", row.selected_for_ace ? "text-success" : "text-muted-foreground")}>
                      ...
                      <td className="whitespace-nowrap px-5 py-4">
                        <div className="flex items-center justify-end gap-1.5">
                          <Button variant="secondary" size="sm" onClick={() => openNotes(row)}>
                            <PenLine className="size-4" aria-hidden="true" />
                            Notes
                          </Button>
                          <Button variant="secondary" size="sm" onClick={() => setSubsTarget(row)}>
                            View submissions
                          </Button>
                        </div>
                      </td>
