...
export function AdminReviews() {
  ...
  return (
    <div className="page py-8">
      ...
      {!loading && !error && data && (
        <>
          <div className="panel grid grid-cols-1 divide-y divide-border md:grid-cols-3 md:divide-x md:divide-y-0">
            ...
            <div className="p-6">
              <p className="font-mono text-[0.6875rem] font-medium uppercase tracking-[0.05em] text-muted-foreground">
                Top tasks
              </p>
              {topTasks.length === 0 ? (
                ...
                <ul className="mt-2 space-y-1.5">
                  {topTasks.map(([name, count]) => (
                    <li key={name} className="flex items-center justify-between gap-3">
                      ...
                      <span className="shrink-0 font-mono text-xs text-electric">{count}</span>
                    </li>
                  ))}
                </ul>
              )}
          ...
          <section className="panel mt-6 panel-ticks relative">
            <div className="grid gap-4 border-b border-border p-5 sm:grid-cols-2 lg:grid-cols-5 sm:p-6">
                ...
                <table className="w-full">
                  ...
                  <tbody className="divide-y divide-border">
                    {filteredStudents.map((p) => {
                      ...
                      return (
                        <tr key={p.id} className="transition-colors duration-150 hover:bg-secondary/60">
                          ...
                          <td className="whitespace-nowrap px-5 py-4">
                            <span className="font-mono text-sm text-foreground">{subs.length}</span>
                          </td>
                          <td className="whitespace-nowrap px-5 py-4 text-right">
                            <Button
                              ...
                              size="sm"
                              onClick={() =>
                                setDialog({ student_id: p.id, title: p.full_name || "Student", email: p.email })
                              }
                            >
          ...
          {dialog && (
            <div className="max-h-[60vh] space-y-3 overflow-y-auto pr-1">
              {(byStudent.get(dialog.student_id) ?? []).length === 0 ? (
                <p className="py-8 text-center text-sm text-muted-foreground">No submissions for this student.</p>
              ) : (
