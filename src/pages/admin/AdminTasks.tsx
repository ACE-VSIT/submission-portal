...
export function AdminTasks() {
  ...
  return (
    <div className="page py-8">
      <Button variant="ghost" size="sm" onClick={() => navigate("/admin/domains")} className="-ml-2 mb-3">
        <ArrowLeft className="size-4" aria-hidden="true" />
        All domains
      </Button>
      <Breadcrumbs items={[{ label: "Admin", to: "/admin" }, { label: "Domains", to: "/admin/domains" }, { label: domain.name }]} />
      ...
      <div className="mb-6 mt-3 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <p className="mt-1.5 max-w-2xl text-sm text-pretty text-muted-foreground">
            {tasks.length} {tasks.length === 1 ? "task" : "tasks"} — create, edit, reorder and control visibility.
          </p>
        </div>
        <Button
          onClick={() => {
            ...
            {tasks.map((task, i) => (
              <li key={task.id} className="flex flex-col gap-3 px-5 py-4 transition-colors duration-150 hover:bg-secondary/60 md:flex-row md:items-center md:gap-4">
                ...
                <div className="flex items-center gap-2 md:w-[110px]">
                  <Switch checked={task.is_visible} onCheckedChange={() => toggleVisibility(task)} aria-label={`Toggle visibility for ${task.name}`} />
                  <Badge variant={task.is_visible ? "success" : "neutral"} className="hidden sm:inline-flex">
