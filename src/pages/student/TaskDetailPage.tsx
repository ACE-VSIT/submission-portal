...
export function TaskDetailPage() {
  ...
  return (
    <div className="page py-8">
      <Button variant="ghost" size="sm" onClick={() => navigate(`/app/domains/${domain.id}`)} className="-ml-2 mb-3">
      ...
      <div className="mb-6 mt-3 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="mb-6 flex flex-wrap items-center gap-x-5 gap-y-2 border-b border-border pb-4">
        ...
        <div className="space-y-6">
          {task.description && (
            <section className="panel p-5 sm:p-6">
              ...
              <p className="text-sm text-pretty leading-relaxed text-foreground">{task.description}</p>
            </section>
          )}
          ...
          <section className="panel p-5 sm:p-6">
            ...
            {task.instructions ? (
              ...
              <p className="text-sm text-muted-foreground">No additional instructions for this task.</p>
            )}
          </section>
        </div>
            ...
            <ul className="space-y-2.5">
              {(task.submission_type === "pdf" || task.submission_type === "pdf_link") && (
                <li className="flex items-start gap-2.5 text-sm text-muted-foreground">
                  <FileText className="mt-0.5 size-4 shrink-0 text-electric" aria-hidden="true" />
                  A single PDF of your work, stored privately.
                </li>
              )}
              {(task.submission_type === "link" || task.submission_type === "pdf_link") && (
                <li className="flex items-start gap-2.5 text-sm text-muted-foreground">
                  <Link2 className="mt-0.5 size-4 shrink-0 text-electric" aria-hidden="true" />
                  One or more links — repository, prototype, demo, or live site.
                </li>
              )}
              <li className="flex items-start gap-2.5 text-sm text-muted-foreground">
                <Clock className="mt-0.5 size-4 shrink-0 text-electric" aria-hidden="true" />
                {task.allows_resubmission
