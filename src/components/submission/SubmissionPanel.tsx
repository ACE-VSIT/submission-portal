...
export function SubmissionPanel({ task, existing, onSuccess }: SubmissionPanelProps) {
  ...
  const canSubmit =
    phase === "idle" && (!needsPdf || pdf !== null) && (!needsLinks || links.length > 0);
  ...
  if (phase === "done" && lastSubmission) {
    return (
      <div className="rounded-sm border border-success/30 bg-success/5 p-6" role="status">
        <div className="flex flex-col items-center gap-2 text-center">
          <CheckCircle2 className="size-8 text-success" aria-hidden="true" />
          <div className="flex items-center gap-2">
            <h3 className="font-heading text-base font-medium text-foreground">Submission confirmed</h3>
          ...
          <p className="max-w-md text-sm text-pretty text-muted-foreground">
            Your work was recorded for <span className="font-medium text-foreground">{task.name}</span>. You can
            resubmit if the task allows it — your latest submission is the one on record.
          </p>
          {lastSubmission.links.length > 0 && (
            <ul className="mt-2 flex flex-wrap items-center justify-center gap-2">
              {lastSubmission.links.slice(0, 3).map((l, i) => (
                <li key={l}>
                  <a
                    ...
                    className="inline-flex max-w-[220px] items-center gap-1 truncate rounded-sm border border-border bg-card px-2 py-1 font-mono text-[0.625rem] uppercase tracking-[0.05em] text-muted-foreground hover:text-electric"
                  >
              ...
              {lastSubmission.links.length > 3 && (
                <li className="font-mono text-[0.625rem] uppercase tracking-[0.05em] text-muted-foreground">
  ...
  return (
    <div className="space-y-5">
      ...
      {phase === "failed" && error && (
        <div className="flex items-center gap-3 rounded-sm border border-error/30 bg-error/5 px-4 py-3" role="alert">
      ...
      <div className="flex items-center justify-between gap-4 border-t border-border pt-5">
        ...
        <Button onClick={handleSubmit} disabled={!canSubmit || busy} loading={busy} className={cn(phase === "failed" && "bg-error")}>
