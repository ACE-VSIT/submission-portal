...
function iconForLink(url: string) {
  if (!url) return <Link2 className="size-4 shrink-0 text-muted-foreground" aria-hidden="true" />;
  const host = url.toLowerCase();
  if (host.includes("github.com")) return <Github className="size-4 shrink-0 text-muted-foreground" aria-hidden="true" />;
  if (host.includes("figma.com")) return <Figma className="size-4 shrink-0 text-muted-foreground" aria-hidden="true" />;
      ...
      {links.length > 0 && (
        <ul className="space-y-2" aria-label="Added links">
          {links.map((link, i) => (
            <li
              key={`${link}-${i}`}
              className="flex items-center gap-2.5 rounded-sm border border-border bg-muted/30 px-3 py-2"
            >
              {iconForLink(link)}
              <a
                href={link}
                ...
                className="min-w-0 flex-1 truncate text-sm text-electric underline-offset-2 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-electric"
              >
                {link}
              </a>
