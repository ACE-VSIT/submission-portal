  ...
  return (
    <div
      className={cn(
        ...
        editable && "transition-colors duration-150 hover:border-muted-foreground/40",
      )}
    >
      <div className="flex flex-wrap items-start justify-between gap-2">
        <div className="min-w-0">
          <p className="text-sm font-medium text-foreground">{submission.task_name}</p>
          <p className="mt-1 flex flex-wrap items-center gap-2 font-mono text-[0.625rem] uppercase tracking-[0.05em] text-muted-foreground">
        ...
        {submission.links.map((link, i) => (
          <a key={link} href={link} target="_blank" rel="noopener noreferrer">
            <Button variant="secondary" size="sm" className="max-w-[280px]">
              ...
              <span className="truncate font-mono text-[0.625rem] uppercase tracking-[0.05em]">
                Link {i + 1}
              </span>
            </Button>
          </a>
