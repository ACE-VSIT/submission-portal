...
export function ErrorState({ title = "Something went wrong", message, onRetry, compact }: ErrorStateProps) {
  return (
    ...
    >
      ...
      {!compact && <h3 className="font-heading text-lg font-medium text-foreground">{title}</h3>}
      <p className={compact ? "text-sm text-error" : "mt-1.5 max-w-md text-sm text-muted-foreground"}>
        {message ?? "An unexpected error occurred. Please try again."}
      </p>
      {onRetry && (
        <Button variant="secondary" size="sm" onClick={onRetry} className={compact ? "ml-auto shrink-0" : "mt-6"}>
